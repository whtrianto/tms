                                    ...
                                    <table id="table-work-activity" class="table table-bordered table-striped text-center w-100">
                                        ...
                                        <tbody>
                                            <?php foreach ($list_data as $key => $value) : ?>
                                                <tr>
                                                    <td><?= $key + 1; ?></td>
                                                    <td><?= $value['WORK_ACTIVITY_ID']; ?></td>
                                                    <td><?= $value['WORK_ACTIVITY_NAME']; ?></td>
                                                    <td><?= $value['WORK_ACTIVITY_DESC']; ?></td>
                                                    <td>
                                                        <div style="display: flex; justify-content: center; gap: 8px;">
                                                            ...
                                                            <button type="button" class="btn btn-danger btn-sm btn-delete"
                                                                data-id="<?= $value['WORK_ACTIVITY_ID'] ?>"
                                                                data-name="<?= htmlspecialchars($value['WORK_ACTIVITY_NAME'], ENT_QUOTES, 'UTF-8') ?>">
                                                                Delete
                                                            </button>
                                                        </div>
                                                    </td>
    ...
    <script>
        $(function() {
            ...
            $("tbody").on('click', '.btn-edit', function(e) {
                ...
                $("input[name=work_activity_id]").val(data.WORK_ACTIVITY_ID);
                $("input[name=work_activity_name]").val(data.WORK_ACTIVITY_NAME);
                $("textarea[name=work_activity_desc]").val(data.WORK_ACTIVITY_DESC);
            ...
            $("tbody").on('click', '.btn-delete', function(e) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    data: { work_activity_id: activityId }
                }).done(function(resp) {
