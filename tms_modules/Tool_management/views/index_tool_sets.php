...
<head>
    ...
    <style>
        ...
        #table-tool-sets th, #table-tool-sets td {
            white-space: nowrap;
        }
        .cell-ellipsis {
            overflow: hidden;
            ...
            <div class="container-fluid" id="container-wrapper">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="m-0 font-weight-bold text-primary">Tool Sets</h4>
                        <div>
                            <a href="<?= base_url('Tool_management/tool_sets/add_page'); ?>" class="btn btn-sm btn-primary shadow-sm">
...
(function($){
    $(function(){
        var table = $('#table-tool-sets').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?= base_url("Tool_management/tool_sets/get_data"); ?>',
                type: 'POST'
            },
        ...
        function attachDeleteHandler() {
            $('#table-tool-sets').off('click', '.btn-delete').on('click', '.btn-delete', function() {
                ...
                if (!confirm('Hapus Tool Set "' + name + '"?')) return;
                
                $.ajax({
                    url: '<?= base_url("Tool_management/tool_sets/delete_data"); ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: { TSET_ID: id }
                }).done(function(res) {
