...
<head>
    ...
    <style>
        ...
        #table-tool-work-order { 
            min-width: 1500px !important; 
        }
        #table-tool-work-order th, #table-tool-work-order td {
            white-space: nowrap;
        }
        .cell-ellipsis {
            overflow: hidden;
            ...
            <div class="container-fluid" id="container-wrapper">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="m-0 font-weight-bold text-primary">Tool Work Order</h4>
                        <div>
                            <a href="<?= base_url('Tool_management/tool_work_order/add_page'); ?>" class="btn btn-sm btn-primary shadow-sm">
...
(function($){
    $(function(){
        var table = $('#table-tool-work-order').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?= base_url("Tool_management/tool_work_order/get_data"); ?>',
                type: 'POST'
            },
        ...
        function attachDeleteHandler() {
            $('#table-tool-work-order').off('click', '.btn-delete').on('click', '.btn-delete', function() {
                var id = Number($(this).data('id')) || 0;
                var wo_no = $(this).data('wo-no') || '';
                if (id <= 0) {
                    alert('ID tidak valid');
                    return;
                }
                if (!confirm('Hapus Work Order "' + wo_no + '"?')) return;
                
                $.ajax({
                    url: '<?= base_url("Tool_management/tool_work_order/delete_data"); ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: { WO_ID: id }
                }).done(function(res) {
