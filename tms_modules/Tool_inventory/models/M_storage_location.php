...
class M_storage_location extends CI_Model
{
    ...
    public function add_data($actor = 'SYSTEM')
    {
        ...
        $desc = trim((string)$this->input->post('storage_location_desc'));

        if ($name === '') {
            $this->messages = "Storage Location Name tidak boleh kosong.";
            return FALSE;
        }
        if ($this->is_duplicate($name)) {
            $this->messages = "Nama lokasi tersebut sudah ada (aktif).";
            return FALSE;
        }
        ...
        if ($this->tms_db->trans_status()) {
            $this->messages = "Data Storage Location berhasil ditambahkan.";
            return TRUE;
        }
    ...
    public function delete_data($id, $actor = 'SYSTEM')
    {
        ...
        $row = $this->get_data_master_storage_location_by_id($id);
        if (!$row) {
