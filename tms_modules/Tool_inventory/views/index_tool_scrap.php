...
<head>
    ...
    <style>
        ...
        #table-tool-scrap th, #table-tool-scrap td {
            white-space: nowrap;
        }
        .cell-ellipsis {
            overflow: hidden;
            ...
            <div class="container-fluid" id="container-wrapper">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="m-0 font-weight-bold text-primary">Tool Scrap</h4>
                        <a href="<?= base_url('Tool_inventory/tool_scrap/add_page'); ?>" class="btn btn-sm btn-primary shadow-sm">
...
(function($){
    $(function(){
        var table = $('#table-tool-scrap').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?= base_url("Tool_inventory/tool_scrap/get_data"); ?>',
                type: 'POST'
            },
        ...
        function attachDeleteHandler() {
            $('#table-tool-scrap').off('click', '.btn-delete').on('click', '.btn-delete', function() {
                var id = Number($(this).data('id')) || 0;
                var scrapNo = $(this).data('scrap-no') || '';
                if (id <= 0) {
                    alert('ID tidak valid');
                    return;
                }
                if (!confirm('Hapus Tool Scrap "' + scrapNo + '"?')) return;
                
                $.ajax({
                    url: '<?= base_url("Tool_inventory/tool_scrap/delete_data"); ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: { SCRAP_ID: id }
                }).done(function(res) {
