...
{
    private $table = 'TMS_M_MATERIAL';
    private $primary_key = 'MATERIAL_ID';
    private $tms_db;
    public $messages;
    ...
    public function get_data_master_materials()
    {
        return $this->tms_db
            ...
            ->from("{$this->table} MAT")
            ->join("TMS_M_UOM UOM", "UOM.UOM_ID = MAT.UOM_ID", 'left')
            ->where('MAT.IS_DELETED', 0) 
            ->order_by('MAT.MATERIAL_NAME', 'ASC')
            ->get()
            ->result_array();
    }
    ...
    public function get_data_master_material_by_name($material_name, $only_active = true)
    {
        ...
        if ($only_active) {
            $sql = "SELECT TOP 1 * FROM {$this->table} WHERE LOWER(MATERIAL_NAME) = ? AND IS_DELETED = 0";
            $params = [strtolower($material_name)];
        } else {
            $sql = "SELECT TOP 1 * FROM {$this->table} WHERE LOWER(MATERIAL_NAME) = ?";
            $params = [strtolower($material_name)];
        }
    ...
    public function is_duplicate($material_name, $exclude_id = null)
    {
        ...
        $this->tms_db->where('LOWER(MATERIAL_NAME)', strtolower($material_name));
    ...
    public function add_data($actor = 'SYSTEM')
    {
        ...
        if ($uom_id <= 0) {
            $this->messages = "UoM wajib dipilih.";
            return FALSE;
        }
        ...
        $data = [
            'MATERIAL_ID'   => $this->get_new_sequence(),
